...
var row =6,col=6; //因後面索引值的取法，陣列最大是10*10
...
for(let r=0;r<row;r++)
{
    ...
    for(let c=0;c<col;c++)
    {
        aryBox[r][c]=Math.floor(Math.random()*row)+","+Math.floor(Math.random()*col)//"r,c" 字串 //將2維陣列放入做隨機的組合排序索引值(會重覆)
        aryVisited[r][c]=0; //產生一個值皆為0的2維陣列
    }
}
